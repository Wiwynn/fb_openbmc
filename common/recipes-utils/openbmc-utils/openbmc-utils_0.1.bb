...
install_sysv() {
    ...
    install -m 0755 ${UNPACKDIR}/dhclient-exit-hooks ${D}${sysconfdir}/dhclient-exit-hooks
    ...
    install -m 0755 ${UNPACKDIR}/rm_poweroff_cmd.sh ${D}${sysconfdir}/init.d/rm_poweroff_cmd.sh
    update-rc.d -r ${D} rm_poweroff_cmd.sh start 99 S .

    install -m 755 ${UNPACKDIR}/setup-reboot.sh ${D}${sysconfdir}/init.d/setup-reboot.sh
    update-rc.d -r ${D} setup-reboot.sh start 89 6 .

    install -m 755 ${UNPACKDIR}/setup-reboot-ongoing.sh ${D}${sysconfdir}/init.d/setup-reboot-ongoing.sh
    ...
    if ! echo ${MACHINE_FEATURES} | awk "/emmc/ {exit 1}"; then
        if [ "x${OPENBMC_UTILS_CUSTOM_EMMC_MOUNT}" = "x0" ]; then
            ...
            install -m 0755 ${UNPACKDIR}/mount_data1.sh \
    ...
    if ! echo ${PACKAGECONFIG} | awk "/disable-watchdog/ {exit 1}"; then
        install -m 0755 ${UNPACKDIR}/disable_watchdog.sh \
...
install_systemd() {
    ...
    install -m 755 ${UNPACKDIR}/eth0_mac_fixup.sh ${D}/usr/local/bin
    install -m 755 ${UNPACKDIR}/setup-reboot.sh ${D}/usr/local/bin
    install -m 755 ${UNPACKDIR}/rc.local ${D}/usr/local/bin
    install -m 755 ${UNPACKDIR}/rc.early ${D}/usr/local/bin
    install -m 644 ${UNPACKDIR}/early.service ${D}${systemd_system_unitdir}
    install -m 644 ${UNPACKDIR}/rm_poweroff_cmd.service ${D}${systemd_system_unitdir}
    # No rm_poweroff_cmd.sh under systemd
    install -m 644 ${UNPACKDIR}/setup-reboot.service ${D}${systemd_system_unitdir}
    install -m 644 ${UNPACKDIR}/mount_data0.service ${D}${systemd_system_unitdir}
    ...
    # Install disable-watchdog.
    if ! echo ${PACKAGECONFIG} | awk "/disable-watchdog/ {exit 1}"; then
        install -m 0755 ${UNPACKDIR}/disable_watchdog.sh ${D}/usr/local/bin
        install -m 0644 ${UNPACKDIR}/disable_watchdog.service \
...
do_install:append:mf-mtd-ubifs() {
    ...
    sed -i 's/FLASH_FS_TYPE=jffs2/FLASH_FS_TYPE=ubifs/' ${UNPACKDIR}/mount_data0.sh
    install -m 0755 ${UNPACKDIR}/data0_resize_ubifs.sh \
...
do_install:append() {
    ...
    for f in ${OPENBMC_PYTHON_LIBS}; do
      install -m 644 ${UNPACKDIR}/$f ${D}${PYTHON_SITEPACKAGES_DIR}/
    done
