...
/**
 * @brief Read the current width of the link
 *
 * @param[in]  link    Pointer to Aries Link struct object
 * @param[out]  currentWidth    Pointer to integer to store the current link
 * width
 * @return     AriesErrorType - Aries error code
 */
...
AriesErrorType ariesGetLinkState(AriesLinkType* link)
{
    ...
    rc = ariesReadByteDataMainMicroIndirect(link->device->i2cDriver,
                                            addressOffset, dataByte);
    CHECK_SUCCESS(rc);
    link->state.state = (AriesLinkStateEnumType) dataByte[0];
...
AriesErrorType ariesLTSSMLoggerClear(AriesLinkType* link)
{
    ...
    dataByte[0] = 0;
    // Main Micro Log clear
    for (offset = 0; offset < ARIES_MM_PRINT_INFO_NUM_PRINT_BUFFER_SIZE;
         offset++)
    {
        baseAddress = link->device->mm_print_info_struct_addr +
                      ARIES_PRINT_INFO_STRUCT_PRINT_BUFFER_OFFSET;
        address = baseAddress + offset;
        rc = ariesWriteByteDataMainMicroIndirect(link->device->i2cDriver,
                                                 address, dataByte);
        CHECK_SUCCESS(rc);
    }

    // Path Micros Log Clear
    int startLane = ariesGetStartLane(link);
    int absLane;
    int laneIndex;
    for (laneIndex = 0; laneIndex < link->state.width; laneIndex++)
    {
        ...
        for (offset = 0; offset < ARIES_PM_PRINT_INFO_NUM_PRINT_BUFFER_SIZE;
             offset++)
        {
            baseAddress = link->device->pm_print_info_struct_addr +
                          ARIES_PRINT_INFO_STRUCT_PRINT_BUFFER_OFFSET;
            address = baseAddress + offset;
            rc = ariesWriteByteDataPathMicroIndirect(
                link->device->i2cDriver, absLane, address, dataByte);
            CHECK_SUCCESS(rc);
        }
    }
...
/**
 * @brief Dump all the debug info from the retimer
 *
 * @param[in]      link    Pointer to Aries Link struct object
 * @return     AriesErrorType - Aries error code
 */
AriesErrorType ariesLinkDumpDebugInfo(AriesLinkType* link)
{
    ...
    if (timeinfo == NULL)
    {
        ASTERA_ERROR("Couldn't get localtime");
        return ARIES_FAILURE;
    }
    strftime(timestamp, 18, "%G%m%d-%H%M", timeinfo);
    ...
    snprintf(filename, 80, "link_state_detailed_bus%d_addr0x%x_link%d_%s",
             link->device->i2cBus, link->device->i2cDriver->slaveAddr,
             link->config.linkId, timestamp);
    rc = ariesLinkPrintDetailedState(link, ".", filename);
    CHECK_SUCCESS(rc);
    // This function prints Aries LTSSM logs to a file
    snprintf(filename, 80, "ltssm_micro_log_bus%d_addr0x%x_link%d_%s",
             link->device->i2cBus, link->device->i2cDriver->slaveAddr,
             link->config.linkId, timestamp);
...
/**
 * @brief Print logs from all micros in retimer to a file
 *
 * @param[in]      link    Pointer to Aries Link struct object
 * @param[in]      basepath    root directory path
 * @param[in]      filename    name of file to write micro logs to
 * @return     AriesErrorType - Aries error code
 */
                                       ...
                                       const char* filename)
{
    ...
    snprintf(filepath, ARIES_PATH_MAX, "%s/%s.py", basepath, filename);
    ...
    for (laneIdx = 0; laneIdx < link->state.width; laneIdx++)
    {
        ...
        rc |= ariesPrintLog(link, (AriesLTSSMLoggerEnumType) laneNum, fp);
...
/**
 * @brief Capture the detailed link state and print it to file
 *
 * @param[in]      link    Pointer to Aries Link struct object
 * @param[in]      basepath    root directory path
 * @param[in]      filename    name of file to write micro logs to
                                           ...
                                           const char* filename)
{
    ...
    for (laneIndex = 0; laneIndex < link->state.width; laneIndex++)
    {
        ...
        int last_preset =
            link->state.usppState.rxState[laneIndex].lastPresetNumReq - 1;
        fprintf(fp, "aries_link['uspp']['rx'][%d]['last_preset_req'] = %d\n",
                laneIndex,
                link->state.usppState.rxState[laneIndex]
                    .lastPresetReq[last_preset]);
        fprintf(fp,
                "aries_link['uspp']['rx'][%d]['last_preset_req_fom'] = %d\n",
                laneIndex,
                link->state.usppState.rxState[laneIndex]
                    .lastPresetReqFom[last_preset]);
        for (i = 1;
             i < link->state.usppState.rxState[laneIndex].lastPresetNumReq; ++i)
        {
            fprintf(
                ...
                laneIndex, i,
                link->state.usppState.rxState[laneIndex]
                    .lastPresetReq[last_preset - i]);
            fprintf(
                fp,
                "aries_link['uspp']['rx'][%d]['last_preset_req_fom_m%d'] = %d\n",
                laneIndex, i,
                link->state.usppState.rxState[laneIndex]
                    .lastPresetReqFom[last_preset - i]);
        }
        fprintf(fp, "\n");
        ...
        last_preset =
            link->state.dsppState.rxState[laneIndex].lastPresetNumReq - 1;
        fprintf(fp, "aries_link['dspp']['rx'][%d]['last_preset_req'] = %d\n",
                laneIndex,
                link->state.dsppState.rxState[laneIndex]
                    .lastPresetReq[last_preset]);
        fprintf(fp,
...
/**
 * @brief Print the micro logger entries
 *
 * Low level api used by ariesLinkPrintMicroLogs() not generally used directly
 *
 * @param[in]      link    Pointer to Aries Link struct object
 * @param[in]      log    log type
 * @param[in]      fp    file pointer to output file for logger entries
 * @return     AriesErrorType - Aries error code
 */
...
/**
 * @brief Set the list of presets that the retimer should request
 *
 * This modifies the list of presets that the retimer will request when
 * performing Phase 2 and 3 of Equalization
 *
 * @param[in]      link    Pointer to Aries Link struct object
 * @param[in]      port    Port to apply new settings to (USPP or DSPP)
 * @param[in]      rate    PCIe rate to apply settings to (Gen-3, Gen-4, or
 *                         Gen-5)
 * @param[in]      laneMask    Logical lane mask of lanes in the link to update
 * @param[in]      presetOneHotList    The one-hot preset request list; set the
 *                                     bit corresponding to the preset you want
 *                                     to enable; for example to request
 *                                     presets 4, 5, and 9 the presetOneHotList
 *                                     would be 0x230
 * @return     AriesErrorType - Aries error code
 */
                                             ...
                                             uint16_t presetOneHotList)
{
    ...
    if ((link->config.partNumber == ARIES_PTX16 &&
         ((port == ARIES_UP_STREAM_PSEUDO_PORT &&
           orientation == ARIES_ORIENTATION_NORMAL) ||
          (port == ARIES_DOWN_STREAM_PSEUDO_PORT &&
           orientation == ARIES_ORIENTATION_REVERSED))) ||
        (link->config.partNumber == ARIES_PTX08 &&
         ((port == ARIES_UP_STREAM_PSEUDO_PORT &&
           orientation == ARIES_ORIENTATION_REVERSED) ||
          (port == ARIES_DOWN_STREAM_PSEUDO_PORT &&
           orientation == ARIES_ORIENTATION_NORMAL))))
    {
        baseAddress = ARIES_MISC_AL_PSEUDO_PORT_0_CSR_OFFSET;
    }
    else if ((link->config.partNumber == ARIES_PTX16 &&
              ((port == ARIES_UP_STREAM_PSEUDO_PORT &&
                orientation == ARIES_ORIENTATION_REVERSED) ||
               (port == ARIES_DOWN_STREAM_PSEUDO_PORT &&
                orientation == ARIES_ORIENTATION_NORMAL))) ||
             (link->config.partNumber == ARIES_PTX08 &&
              ((port == ARIES_UP_STREAM_PSEUDO_PORT &&
               ...
               (port == ARIES_DOWN_STREAM_PSEUDO_PORT &&
                orientation == ARIES_ORIENTATION_REVERSED))))
    {
        baseAddress = ARIES_MISC_AL_PSEUDO_PORT_1_CSR_OFFSET;
    }
    else
    {
        return ARIES_INVALID_ARGUMENT;
    ...
    else if (rate == ARIES_GEN4)
    {
        rateOffset = 0x2;
    }
    else if (rate == ARIES_GEN3)
    {
        rateOffset = 0x4;
    }
    else
    ...
    startLane = ariesGetStartLane(link);
    for (laneIndex = 0; laneIndex < link->state.width; laneIndex++)
    {
        if ((laneMask >> laneIndex) & 0x1)
        {
            absLane = startLane + laneIndex;
            ...
            sliceOffset = (absLane / 2) * 0x2e;

            address = baseAddress + sliceOffset + laneOffset + rateOffset;

            dataWord[0] = presetOneHotList & 0xff;
            dataWord[1] = (presetOneHotList >> 8) & 0x7;
            rc = ariesWriteWideRegister(link->device->i2cDriver, address, 2,
                                        dataWord);
            CHECK_SUCCESS(rc);
        }
    }
