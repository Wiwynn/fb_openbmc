    ...
    auto run(sdbusplus::async::context& ctx) -> sdbusplus::async::task<>
    {
        ...
        co_await subtree_for_each(
            ...
            [&](const auto& path,
                ...
                auto pwm = control::fan_pwm::Proxy(ctx).service(service).path(
                    path.str);
                ...
                // This is a hack, but how dbus-sensor's FanSensor currently
                // works. They don't have any association but they at least use
                // the same final element name.
        ...
        co_await subtree_for_each(
            ctx, fan_tach_subpath, sensor::interface,
            ...
            [&](const auto& path,
                ...
                // We need to strip off the _ from _TACH and make it lowercase
                // for a key.
