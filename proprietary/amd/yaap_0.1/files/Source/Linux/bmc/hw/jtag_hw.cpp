...
int bmc::JtagHw::default_jtag_xfer_mode = JTAG_XFER_SW_MODE;
...
bmc::JtagHw::JtagHw(int driver)
    : m_state(yaap::hal::JTAG_STATE_TLR), m_freq(8250000)
{
    int rc;
    unsigned int freq = 0;
...
uint32_t bmc::JtagHw::shift(enum yaap::hal::jtagOp operation, uint8_t *inData, uint8_t *outData, int bits, [[maybe_unused]] int preTck, [[maybe_unused]] int postTck, enum yaap::hal::jtagState termState)
{
    int test;
    int remain_bits;
...
#ifdef  YAAP_ENTRY_DEBUG
    DEBUG_PRINT(DEBUG_INFO, "yaap bmc::JtagHw::shift entry !!!.\n");
#endif

    DEBUG_PRINT(DEBUG_INFO, "YAAP IOCTL SHIFT Operation:%d, BITs Count: 0x%x.\n", operation, bits);
    ...
    if (outData) {
      for (int i = 0; i < BYTECOUNT(remain_bits); ++i)
        outData[i] = ((uint8_t *)xfer.tdio)[i];
    }

    message = "JtagHw::shift Output Data";
    printx(message.c_str(), outData, bytes);
