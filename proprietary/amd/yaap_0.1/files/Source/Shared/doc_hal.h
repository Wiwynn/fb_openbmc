 ...
 *
 * This documentation covers the API for the hardware abstraction layer (HAL) of the YAAP 
 * server software.  YAAP is a a remote procedure call protocol used for communication with 
 * debug modules connected to AMD silicon targets.  
 *
 ...
 *
 * \par Debug Host: 
 * This is a computer running compatible YAAP client debug software, such as AMD HDT or AMD ACE.
 *
 * \par YAAP Core:
 ...
 * The Hardware Abstraction Layer (HAL) is the API defined in this documentation, and it represents
 * the interface between the common YAAP Core code and the implementation-specific low-level 
 * code, which knows how to interface with the debug module hardware.
 * <br><br>
