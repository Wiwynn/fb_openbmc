...
{
    double maxFreq = 1 / 4.04040404e-8;     // !!! Max freq currently hard coded to 12.375 or 1/8.08080e-8 MHz
    uint32_t freq = 0;
    uint32_t retval = E_NOT_IMPLEMENTED;
    ...
    if (m_tckFreq != NULL)
    {
        retval = m_tckFreq->tckFreqGet(freq);

  DEBUG_PRINT(DEBUG_INFO, "Returned freq = %u", freq); 
	double divisor = (24750000/freq);
...
uint32_t jtag::yaap_optionTckFrequencySet(YAAP_METHOD_PARAMS)
{
	...
	uint32_t freq = 24750000/ ((0x7ff & divisor) + 1);
