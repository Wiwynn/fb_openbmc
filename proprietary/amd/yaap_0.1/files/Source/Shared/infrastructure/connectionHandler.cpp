...
namespace yaap {
    ...
    inline uint32_t connectionHandler::timeDiffMsec(const struct timeval& first, const struct timeval& second)
    {
...
#ifdef LIBYAAP_BIC
        std::vector<uint8_t> bic_buff_t0;
        std::vector<uint8_t> bic_buff_t1;
        yaap_bic::BicWrapBackend bic_backend(yaap_bic::BicWrapConfig::getInstancePtr()->getFruId());
#endif
...
//            writeBytesToDebugFile(rxBuf + rxBytes, bytesToProcess);
            ...
            while (bytesToProcess && (state != DONE))
            {
                ...
                {
                    // The element currently being received straddles this packet and the next.
                ...
                else
                {
                    ...
                    case METHOD_CNT:
                        ...
                        if (methodCounter > 0)
                        {
                            ...
                            remainingBytesInState = YAARP_SIZE_INT;
                            DEBUG_PRINT(DEBUG_VERBOSE, "State METHOD_CNT (%u) -> METHOD_INSTANCE_ID", methodCounter);
                    ...
                    case METHOD_INSTANCE_ID:
                        ...
                        remainingBytesInState = YAARP_SIZE_INT;
                        DEBUG_PRINT(DEBUG_VERBOSE, "State METHOD_INSTANCE_ID (%u) -> METHOD_METHOD_ID", tmpInt);
                        break;
                    ...
                    case METHOD_METHOD_ID:
                        ...
                        remainingBytesInState = YAARP_SIZE_INT;
                        DEBUG_PRINT(DEBUG_VERBOSE, "State METHOD_METHOD_ID (%u) -> METHOD_PAYLOAD_SIZE", tmpInt);
                        break;
                    ...
                    case METHOD_PAYLOAD_SIZE:
                        ...
                        if (methodPayloadSize)
                        {
                            ...
                            DEBUG_PRINT(DEBUG_VERBOSE, "State METHOD_PAYLOAD_SIZE (%u) -> METHOD_PAYLOAD",  tmpInt);
                            break;
                        }
                        // FALLTHROUGH
...
//            PROF_PIN_SET(CONN_PRF_YAARP_ALLOC);
        ...
        bic_buff_t0.clear();
        bic_buff_t1.clear();
        uint32_t totalPages = yaap_bic::BicWrapBackend::checkTotalPages(yaap_bic::BasicDataBuffer(rxBufBase, (rxBufBase+rxBytes)));
        uint16_t currPage = 0;
#endif
        ...
        DEBUG_PRINT(DEBUG_MOREINFO, "Lock ID = %08X, Method count = %u", lockId, methodCnt);

#ifdef LIBYAAP_BIC
        if (bic_backend.sendYaapHeader(lockId, methodCnt, totalPages, currPage) != E_SUCCESS)
        {
            DEBUG_PRINT(DEBUG_ERROR, "BIC invokeHeader error");
            es.add(E_SERVER_ERROR, "BIC invokeHeader error");
        }
#endif
        if (is.isError())
        {
        ...
        else
        {
            for (methodsRun = 0; methodsRun < methodCnt; )
            {
                ...
                if (is.isError())
                {
                    DEBUG_PRINT(DEBUG_ERROR, "Input stream error before call # %u", methodsRun);
                    es.add(E_MALFORMED_REQUEST, "Error reading input before method call");
                    break;
                }
                else if (!base::isValidInstance(instanceId))
                {
                    snprintf(tmpStringBuf, TMP_STRING_BUF_SIZE, "Bad instance ID (%u) in method call %u", instanceId, methodsRun);
                    DEBUG_PRINT(DEBUG_ERROR, "Bad instance ID (%u) in method call %u", instanceId, methodsRun);
                    es.add(E_INVALID_INSTANCE_ID, tmpStringBuf);
                    break;
                }
                else if (!base::isValidMethod(instanceId, methodId))
                {
                    snprintf(tmpStringBuf, TMP_STRING_BUF_SIZE, "Bad method ID (%u.%u) in method call %u", instanceId, methodId, methodsRun);
                    DEBUG_PRINT(DEBUG_ERROR, "Bad method ID (%u:%u) in method call %u", instanceId, methodId, methodsRun);
...
#ifdef LIBYAAP_BIC
                uint32_t methodRet;
                uint8_t *payloadDataPtr = is.getRawBytePtr(payloadSize);
                if (bic_backend.sendYaapMethodRequest(instanceId, methodId, payloadSize, yaap_bic::BasicDataBuffer(payloadDataPtr, (payloadDataPtr+payloadSize)), totalPages, currPage) != E_SUCCESS)
                {
                    DEBUG_PRINT(DEBUG_ERROR, "BIC sendYaapMethodRequest error");
                    es.add(E_SERVER_ERROR, "BIC sendYaapMethodRequest error");
                    break;
                }
                else if (bic_backend.getYaapMethodResponse(bic_buff_t0, 0) != E_SUCCESS)
                {
                    DEBUG_PRINT(DEBUG_ERROR, "BIC getYaapMethodResponse output error");
                    es.add(E_SERVER_ERROR, "BIC getYaapMethodResponse output error");
                    break;
                }
                else
                {
                ...
                {
                    DEBUG_PRINT(DEBUG_ERROR, "Method returned %u; terminating multicall; %u of %u methods executed", methodRet, methodsRun, methodCnt);
                ...
                {
                    DEBUG_PRINT(DEBUG_ERROR, "Expected method to consume %u bytes, but it consumed %zu", payloadSize, bytesBefore - is.getBytesRemaining());
                ...
                if (is.isError())
                {
                    DEBUG_PRINT(DEBUG_WARNING, "Input stream error after call # %u", methodsRun);
                    es.add(E_MALFORMED_REQUEST, "Error in input stream after method call");
                    break;
                }
                if (os.isError())
                {
                    DEBUG_PRINT(DEBUG_WARNING, "Output stream error after call # %u", methodsRun);
...
#ifdef LIBYAAP_BIC
        if (bic_backend.getYaapMethodResponse(bic_buff_t1, 1) != E_SUCCESS)
        {
            DEBUG_PRINT(DEBUG_ERROR, "BIC getYaapMethodResponse status error");
            es.add(E_SERVER_ERROR, "BIC getYaapMethodResponse status error");
        }
#endif
...
#ifdef LIBYAAP_BIC
        if (!yaap_bic::sendRawData(fd, bic_buff_t1))
        {
            DEBUG_PRINT(DEBUG_ERROR, "Send bic_buff_t1 failed");
            return KEEP_OPEN;
        }

        if (!yaap_bic::sendRawData(fd, bic_buff_t0))
        {
            DEBUG_PRINT(DEBUG_ERROR, "Send bic_buff_t0 failed");
            return KEEP_OPEN;
        }
#else
