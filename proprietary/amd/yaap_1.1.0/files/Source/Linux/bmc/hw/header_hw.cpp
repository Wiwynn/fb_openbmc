...
uint32_t bmc::HeaderHw::setEnabled(bool enabled)
{
...
uint32_t bmc::HeaderHw::trstSet(bool reset)
{
    ...
    value = (reset == true)? 0:1;

    test = setGPIOValue(jtagTRSTName, value);
    if(test == false)
    {

       DEBUG_PRINT(DEBUG_ERROR, "HeaderHw::trstSet Low failed, TRST reset = %d", reset);
