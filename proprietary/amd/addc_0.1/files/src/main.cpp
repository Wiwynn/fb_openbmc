...
void dump_context_info(uint16_t numbanks,uint16_t bytespermca,uint8_t info)
{
    ...
    if(num_of_proc == TWO_SOCKET)
    {
        set_apml_mux_locked(p1_info);
        getLastTransAddr(p1_info);
        harvestPcieDump(p1_info);
    }
    set_apml_mux_locked(info);
...
bool harvest_ras_errors(uint8_t info,std::string alert_name)
{
    ...
    if (ras_status || read_sbrmi_ras_status(info, &buf) == OOB_SUCCESS)
    {
        if (ras_status) {
            buf = ras_status;
        }
        ...
        if (buf & INT_15)
        {
            ...
            if (ResetReady == true)
            {
                ...
                {
                    if (buf & (SYS_MGMT_CTRL_ERR | RESET_HANG_ERR))
                    {
                        triggerColdReset();
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < cpuid_str_vec.size(); ++i) {
    ...
    if (num_of_proc == TWO_SOCKET) {
        for (target_cpu = 0; target_cpu < num_of_proc; ++target_cpu) {
            ...
            harvest_ras_errors(target_cpu, alertName);
        }
    } else {
        std::string alertName = target_cpu ? "P1_ALERT" : "P0_ALERT";
        harvest_ras_errors(target_cpu, alertName);
