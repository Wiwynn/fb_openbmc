...
class CpxReport : public Report
{
  public:
    CpxReport() = delete;
    CpxReport(Summary& summary, CpxTOR& tor) : Report(summary), tor(tor) {};
    ...
    [[nodiscard]] json createJSONReport() {
        ...
        for (auto const& [socketId, reportData] : tor)
        {
            ...
            errorsPerSocket["Errors Per Socket"] =
                createSummaryString(ctxSocket, CpxfirstError);
            ...
            for (const auto& torEntry : tors)
            {
                ...
                if (static_cast<uint8_t>(torEntry.lcs) < CPX_LLCS.size())
                {
                    details["LLC.decoded"] = CPX_LLCS[torEntry.lcs];
                }
                details["Target"] =
                    int_to_hex(static_cast<uint8_t>(torEntry.target));
                if (static_cast<uint8_t>(torEntry.target) < CPX_PORT_ID.size())
                {
                    details["Target.decoded"] = CPX_PORT_ID[torEntry.target];
                }
                ...
                if (torEntry.sad == static_cast<uint8_t>(SadValues::CFG))
                {
                    ...
                    if (static_cast<uint8_t>(torEntry.target) <
                        CPX_PORT_ID.size())
                    {
                        decoded["Port"] = CPX_PORT_ID[torEntry.target];
                    }
                    decoded["BDF"] =
                ...
                auto firstMcerrCha = getFirstErrorCha(
                    CpxfirstErrorCha, ctxSocket.mcerr.firstMcerrSrcIdCha);
                ...
                auto firstIerrCha = getFirstErrorCha(
                    CpxfirstErrorCha, ctxSocket.ierr.firstIerrSrcIdCha);
