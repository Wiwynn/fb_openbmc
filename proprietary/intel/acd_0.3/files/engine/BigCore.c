                               ...
                               uint32_t crashdumpVerSize)
{
    ...
    uint32_t size = getCrashDataSectionBigCoreSize(
        cpuInfo->inputFile.bufferPtr, jsonItemName, crashdumpVerSize);
                           ...
                           uint32_t u32version)
{
    ...
    cJSON* sizeArray = getCrashDataSectionBigCoreSizeArray(
    ...
    return (
        ...
        ((totalInputSize == (uCrashdumpVerSize.field.regDumpSize)) ||
         (isValueInJsonArray(sizeArray, uCrashdumpVerSize.field.regDumpSize))));
}
    ...
    uint8_t logicalDieNum)
{
    ...
    if (isStoreDecoded(cpuInfo, totalInputSize, uCrashdumpVerSize, u32version))
    {
        switch (cpuInfo->model)
        {
            ...
            case cd_icx2:
            case cd_icxd:
                // Todo Remove once we clean ICX processing
                crashdumpJsonICX(
 ...
 *
 ******************************************************************************/
acdStatus logCrashdumpSPRGNR(CPUInfo* cpuInfo, cJSON* pJsonChild,
                            ...
                            uint8_t logicalDieNum)
{
    ...
    logCrashdumpSPRGNR(cpuInfo, pJsonChild, runTimeInfo, domainID,
                       logicalDieNum);
    return ACD_SUCCESS;
}
