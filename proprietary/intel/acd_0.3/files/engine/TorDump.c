                     ...
                     uint8_t logicalDieNum)
{
    ...
    // Crashdump Get Frames
    for (size_t cha = 0; cha < cpuInfo->computeDies[logicalDieNum].chaCount;
         cha++)
    {
        for (uint32_t u32TorIndex = 0; u32TorIndex < TD_TORS_PER_CHA_ICX1;
             u32TorIndex++)
                 ...
                 u32TorSubIndex < TD_SUBINDEX_PER_TOR_ICX1; u32TorSubIndex++)
            {
                ...
                if (!skipCha)
                {
                    ret = peci_CrashDump_GetFrame_dom(
                        cpuInfo->clientAddr, domainID, PECI_CRASHDUMP_TOR, cha,
                        (u32TorIndex | (u32TorSubIndex << 8)), u8PayloadBytes,
                        pu8TorCrashdumpData, &cc);
...
static const STorSectionVx sTorSectionVx[] = {
    ...
    {cd_sprhbm, logTorSectionICXSPR}, {cd_gnr, logTorSectionGNR},
    {cd_emr, logTorSectionICXSPR},    {cd_srf, logTorSectionGNR}};
