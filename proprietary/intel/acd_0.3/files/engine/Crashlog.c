...
struct pmtHandler pmtbind = {NULL, NULL, NULL};
#endif
                                       ...
                                       const uint8_t domainID)
{
    ...
    ret = peci_Telemetry_ConfigWatcherRd_dom(
        cpuInfo->clientAddr, domainID, CONFIG_WATCHER_ID, CONFIG_WATCHER_OFFSET,
                        ...
                        const agentsInfoInputFile* const agentsInfo)
{
    ...
    if (error)
    {
        char jsonErrorString[CRASHLOG_ERROR_JSON_STRING_LEN] = {0};
        cd_snprintf_s(jsonErrorString, CRASHLOG_ERROR_JSON_STRING_LEN,
                      "Could not encode full crashlog (error=0x%lx) for "
                      "agent=0x%x, uniqueId=0x%x",
                            ...
                            RunTimeInfo runTimeInfo)
{
         ...
         sampleId++)
    {
        ...
        ret = peci_Telemetry_GetCrashlogSample_dom(
            cpuInfo->clientAddr, domainID, agent, sampleId, sizeof(uint64_t),
            (uint8_t*)&data, &cc);
        if (ret != PECI_CC_SUCCESS || (PECI_CC_UA(cc)))
        {
            CRASHDUMP_PRINT(
                                                ...
                                                const uint8_t domainID)
{
    ...
    ret = peci_Telemetry_Discovery_dom(cpuInfo->clientAddr, domainID, 2, 0x4,
                                       agent, 0, sizeof(crashlogDetails),
                                   ...
                                   const uint8_t domainID)
{
    ...
    ret = peci_Telemetry_Discovery_dom(cpuInfo->clientAddr, domainID, 1, 0x4, 0,
...
bool isTelemetrySupported(const CPUInfo* const cpuInfo, const uint8_t domainID)
{
    ...
    uint8_t cc;
    EPECIStatus ret = peci_Telemetry_Discovery_dom(
        cpuInfo->clientAddr, domainID, 0, 0, 0, 0, sizeof(telemetrySupported),
...
bool isCpuCrashlogEnabled(const uint8_t domainID)
{
    uint32_t data;
    uint8_t cc;
    EPECIStatus ret = peci_Telemetry_ConfigWatcherRd_dom(
        MIN_CLIENT_ADDR, domainID, CONFIG_WATCHER_ID, CONFIG_WATCHER_OFFSET,
        sizeof(data), &data, &cc);
    if (ret != PECI_CC_SUCCESS || (PECI_CC_UA(cc)))
    {
        CRASHDUMP_PRINT(ERR, stderr,
...
bool isCpuCrashlogEnabled_dom(const uint8_t domainID)
{
    uint32_t data;
    uint8_t cc;
    EPECIStatus ret = peci_Telemetry_ConfigWatcherRd_dom(
...
#ifdef PMT_NAC_CRASHLOG
void registerPMTCallback(struct pmtHandler* pmtHandleBack)
{
    if (pmtHandleBack->pmtObjptr == NULL || pmtHandleBack->ioc == NULL ||
        pmtHandleBack->fn == NULL)
    {
        return;
    }
    pmtbind = *pmtHandleBack;
}
#endif
                      ...
                      RunTimeInfo runTimeInfo)
{
    ...
    bool crashlogEnable = false;
    if (cpuInfo->dieMaskInfo.dieMaskSupported)
    {
        crashlogEnable = isCpuCrashlogEnabled_dom(domainID);
    }
    else
    {
        crashlogEnable = isCpuCrashlogEnabled(domainID);
    }
    if (crashlogEnable)
    {
    ...
    CRASHDUMP_PRINT(INFO, stderr, "Found %d cl agent(s) from input file\n",
                    agentsInfo.numberOfCrashlogAgents);
    CRASHDUMP_PRINT(
        INFO, stderr,
        "Found %d cl agent(s) from discovery on PECI address %d domain %d\n",
        agentsNum, cpuInfo->clientAddr, domainID);
    ...
    {
        crashlogAgentDetails agentDetails =
            getCrashlogDetailsForAgent(cpuInfo, agent, domainID);
        ...
        {
            CRASHDUMP_PRINT(ERR, stderr,
                            "cl agent id:(0x%x) (idx:%d) not found in "
                            "input file on PECI address %d domain %d\n",
                            agentDetails.uniqueId, agent, cpuInfo->clientAddr,
                            domainID);
        }
        ...
        error = collectCrashlogForAgent(cpuInfo, agent, &agentDetails,
                                        rawCrashlog, domainID, runTimeInfo);
        ...
        else
        {
            ...
            if (storeCrashlog(pJsonChild, agent, &agentDetails, rawCrashlog,
                              &agentsInfo) != ACD_SUCCESS)
            {
                char jsonItemName[NAME_STR_LEN] = {0};
                cd_snprintf_s(jsonItemName, NAME_STR_LEN,
                              "crashlog_store_fail_agent_0x%x", agent);
                logInOutput(pJsonChild, jsonItemName, "true");
            }
        }
...
#ifdef PMT_NAC_CRASHLOG
    // Collect NAC_PMT based crashlog
    if (pmtbind.fn && pmtbind.ioc && pmtbind.pmtObjptr)
    {
        pmtbind.fn(pJsonChild, &pmtbind);
    }
#endif
    ...
    if (isCrashlogExtracted)
    {
        ...
        if (initiateCrashlogTriggerRearm(cpuInfo, domainID) != ACD_SUCCESS)
        {
...
acdStatus logCrashlogSection(const CPUInfo* const cpuInfo,
                             ...
                             const uint16_t totalAgents, const uint8_t domainID,
                             RunTimeInfo runTimeInfo)
{
    return logCrashlog(cpuInfo, outputNode, totalAgents, domainID, runTimeInfo);
}

static const CrashlogVx crashlogVx[] = {
    {cd_spr, logCrashlog},
    {cd_sprhbm, logCrashlog},
};
