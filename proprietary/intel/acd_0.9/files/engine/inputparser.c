...
void ReadLoops(cJSON* section, LoopOnFlags* loopOnFlags)
{
    ...
    loopOnFlags->loopOnModule = false;
    ...
    LoopOn = cJSON_GetObjectItemCaseSensitive(section, "LoopOnModule");
    if (LoopOn != NULL)
    {
        if (cJSON_IsBool(LoopOn) && cJSON_IsTrue(LoopOn))
        {
            loopOnFlags->loopOnModule = true;
        }
    }
}
    ...
    for (int pos = 0; pos < cJSON_GetArraySize(cmdInOut->in.params); pos++)
    {
        ...
        if (cJSON_IsString(param))
        {
            ...
            if (internalVar != NULL)
            {
                ...
                if (IsValidHexString(internalVar->valuestring))
                {
                    cJSON_ReplaceItemInArray(
                        cmdInOut->in.params, pos,
                        cJSON_CreateNumber((double)strtoull(
                            internalVar->valuestring, NULL, 16)));
                }
                else
                {
                    cJSON_ReplaceItemInArray(
                        cmdInOut->in.params, pos,
                        cJSON_CreateString(internalVar->valuestring));
                }
            }
            else
            {
                ...
                strcmp_s(INSTANCE, strnlen_s(INSTANCE, sizeof(INSTANCE)),
                         param->valuestring, &mismatchInstance);
                ...
                else if (mismatchcore == 0)
                {
                    ...
                    cJSON_ReplaceItemInArray(
                        cmdInOut->in.params, pos,
                        cJSON_CreateNumber(
                            loggerStruct->contextLogger.logicalCore));
                }
                ...
                else if (mismatchInstance == 0)
                {
                    cJSON_AddItemToObject(cmdInOut->paramsTracker, INSTANCE,
                                          cJSON_CreateNumber(pos));

                    // This will be updated later during ExecuteModule()
                    if (pos < MAX_INPUT_ARG_COUNT)
                    {
                        cmdInOut->rmSectionInfo->instanceArgumentIndex = pos;
                    }
                ...
                else if (mismatchIoDomain == EOK)
                {
                    ...
                    mappedDie =
                        mapDieNumToBit(cpuInfo->dieMaskInfo.dieMask, ioDomain,
                    ...
                    loggerStruct->contextLogger.isDomainCompute =
                        isDomainCompute(&cpuInfo->dieMaskInfo,
                                        loggerStruct->contextLogger.domainID);
                }
        ...
        // Special Input file parsing for RM;
        if (cmdInOut->rmSectionInfo != NULL &&
            cmdInOut->rmSectionInfo->isRemoteSection)
        {
            // Retrieve value again and check for strings
            cJSON* param = cJSON_GetArrayItem(cmdInOut->in.params, pos);
            RemoteModuleInfo* rmInfo;
            rmInfo = cmdInOut->rmSectionInfo->rmModuleInfo;
            if (cJSON_IsBool(param))
            {
                if (rmInfo->boolCount < MAX_INPUT_ARG_COUNT)
                {
                    rmInfo->boolArg[rmInfo->boolCount] = cJSON_IsTrue(param);
                    rmInfo->boolCount += 1;
                }
                else
                {
                    CRASHDUMP_PRINT(
                        ERR, stderr,
                        "Too many bool arguments in Module Section\n");
                }
            }
            else if (cJSON_IsString(param))
            {
                if (rmInfo->strCount < MAX_INPUT_ARG_COUNT)
                {
                    rmInfo->strArg[rmInfo->strCount] = param->valuestring;
                    rmInfo->strCount += 1;
                }
                else
                {
                    CRASHDUMP_PRINT(
                        ERR, stderr,
                        "Too many str arguments in Module Section\n");
                }
            }
            else if (cJSON_IsNumber(param))
            {
                if (rmInfo->intCount < MAX_INPUT_ARG_COUNT)
                {
                    rmInfo->intArg[rmInfo->intCount] = param->valueint;
                    rmInfo->intCount += 1;
                }
                else
                {
                    CRASHDUMP_PRINT(
                        ERR, stderr,
                        "Too many int arguments in Module Section\n");
                }
            }
        }
    }
