                        ...
                        const agentsInfoInputFile* const agentsInfo)
{
    ...
    getAgentName(agentDetails, (char*)&sectionNameString,
...
bool isCpuCrashlogEnabled_dom(const uint8_t domainID, const int clientAddr)
{
    uint32_t data;
    uint8_t cc;
    EPECIStatus ret = peci_Telemetry_ConfigWatcherRd_dom(
        clientAddr, domainID, CONFIG_WATCHER_ID, CONFIG_WATCHER_OFFSET,
        sizeof(data), (uint8_t*)&data, &cc);
    if (ret != PECI_CC_SUCCESS || (PECI_CC_UA(cc)))
    {
        CRASHDUMP_PRINT(ERR, stderr,
...
acdStatus logCrashlog(const CPUInfo* const cpuInfo, cJSON* const pJsonChild,
                      uint16_t agentsNum, const uint8_t domainID,
                      RunTimeInfo runTimeInfo)
{
    cJSON* agents =
    ...
    // Notes: only domain 0 is supported for crashlog enable check.
    bool crashlogEnable = isCpuCrashlogEnabled_dom(0, cpuInfo->clientAddr);
    if (crashlogEnable)
    {
...
void logNACPMTCrashlog(cJSON* pJsonChild)
{
#ifdef PMT_NAC_CRASHLOG
