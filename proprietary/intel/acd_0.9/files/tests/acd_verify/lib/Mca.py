...
class Mca(Section):
        ...
        if type(value) == dict:
            ...
            if valueIsError:
                ...
                if 'upi' in key:
                    ...
                    upiBank = lastKey.split('_')[0]
                    self.upiBanks[upiBank] &= False
    ...
    def searchSOC(self, key, value):
        if 'UPI' in key:
            lastKey = (key.split(".")[-1]) if ("." in key) else key
            self.upiBanks[lastKey.lower()] = True

        keyIsMemReg = ((key.startswith('MSE')) or
                       (key.startswith('MCHN')) or
                       (key.startswith('B2CMI')))
        if type(value) == dict:
            for vKey in value:
                self.searchSOC(f"{key}.{vKey}", value[vKey])
        elif (type(value) == str) and not value.startswith('_'):
            self.nRegsSoc += 1
            valueIsError = self.eHandler.isError(value)
            if valueIsError:
                error = self.eHandler.extractError(value)
                if "soc" not in self.eHandler.errors:
                    self.eHandler.errors["soc"] = {}
                if keyIsMemReg:
                    self.eHandler.errors["soc"][key] = [error, "keyIsMemReg"]
                else:
                    self.eHandler.errors["soc"][key] = error
    ...
    def verifySection(self):
        for key in self.sectionInfo:
            keyIsSOCReg = ((key.startswith('MSE')) or
                        (key.startswith('MCHN')) or
                        (key.startswith('B2CMI')) or
                        (key.startswith('LLC')) or
                        (key.startswith('CBO')) or 
                        (key.startswith('MC')) or 
                        (key.startswith('UPI'))) 
            isExpectedRegs = ((key.startswith('_')) or
                              (key.startswith('module')) or
                              keyIsSOCReg)
    ...
    def getTableInfoCore(self):
        ...
        return tableInfo if self.nRegsCores > 0 else None
    ...
    def getTableInfoUncore(self):
        ...
        return tableInfo if self.nRegsUncore > 0 else None

    def getTableInfoSoc(self):
        tableInfo = {
            "Section": "MCA_soc",
            "rootNodes": "",
            "regs": self.nRegsSoc
        }
        if "soc" in self.eHandler.errors:
            errors = self.eHandler.errors["soc"]
            haveCmts = any(type(x) == list for x in errors.values())
            errorValues = \
                [x[0] if (type(x) == list) else x for x in errors.values()]
            level = dict(collections.Counter(errorValues))
            for error in level:
                if error not in tableInfo:
                    tableInfo[error] = level[error]
                elif error in tableInfo:
            ...
            if haveCmts:
                cmt = ""
                cmtTypes = [x for x in errors.values() if (type(x) == list)]
                cmtTypes = set([cmt[1] for cmt in cmtTypes])
                if 'keyIsMemReg' in cmtTypes:
                    cmt += "Errors dependant on DIMM being populated."
                tableInfo["comments"] = cmt

        return tableInfo if self.nRegsSoc > 0 else None

    def getTableInfo(self):
        return [self.getTableInfoCore(),
                self.getTableInfoUncore(),
                self.getTableInfoSoc()]
    ...
    def getErrorList(self):
        ...
        if "soc" in self.eHandler.errors:
            uncoreErrors = self.eHandler.errors["soc"]

        return [coreErrors, uncoreErrors, socErrors]
    ...
    def getUpiEnCount(self):
        count = str(self.upiBanks).count('True')
