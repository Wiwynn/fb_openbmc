                ...
                if key == "metadata":
                    ...
                    for die in request["sections"][key]:
                        ...
                        self.processSectionOf(
                            ...
                            errorList[key][die],
                            report["timeInfo"][key][die],
                            True, sectionsUPICount)
                        
                        if len(sectionsUPICount) > 0:
                            cpuUPICounts[die] = sectionsUPICount
                    if len(cpuUPICounts) > 0:
                        upiCounts[key] = cpuUPICounts
                        report["upiCountCheck"] = upiCounts

            report["selfCheck"] = errorList
            return True

    def processSectionOf(self, requestSections, errorListSections,
                         socketTimeInfo, doUPICheck=False, upiCounts=None):
        for section in requestSections:
            ...
            hasUpiEnCount = hasattr(sectionObj,
                                    'getUpiEnCount')
            if hasUpiEnCount and doUPICheck:
                upiCounts[section] = sectionObj.getUpiEnCount()
