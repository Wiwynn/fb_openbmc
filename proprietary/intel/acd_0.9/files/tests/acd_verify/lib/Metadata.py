...
class Metadata(Section):
    def __init__(self, jOutput, checks=None, dies=None):
        ...
        _version = jOutput["METADATA"].get("_version", "0x00")
        self.cpus = []
        self.dies = dies
        self.verifySection()
    ...
    def getSummaryInfo(self):
        ...
        summaryInfo["totalTime"] = self.sectionInfo.get("_total_time", "Not present")
        summaryInfo["triggerType"] = self.sectionInfo.get("trigger_type", "Not present")
        ...
        if self.dies:
            for cpu in self.dies:
                ...
                for compute in self.dies[cpu]["computes"]:
                    if "final_crashcore_count" in self.sectionInfo[cpu][compute]:
                        counts[cpu][compute] = self.sectionInfo[cpu][compute]["final_crashcore_count"]
                    else:
                        counts[cpu][compute] = self.sectionInfo[cpu][compute]["crashcore_count"]
        else:
            for cpu in self.cpus:
    ...
    def checkCpuID(self, cpu):
        ...
        if cpuID:
            ...
            _validCPUIDs = ("0X5065", "0X606A", "0X806F",
                            "0XA06D", "0XA06F", "0XC06F", "0XA06E")
            if (cpuID[:-1] not in _validCPUIDs):
    ...
    def getTimeInfo(self):
        ...
        if "_time" in self.sectionInfo:
            ...
            ret = {}
            total_meta = 0
            for k, v in self.sectionInfo.items():
                if k.startswith("_time_"):
                    ret[k] = float(v.strip('s'))
                    total_meta += ret[k]
            ret["_total_time"] = self.sectionInfo.get("_total_time", "Not present")
            ret["TOTAL"] = total_meta
            return ret
