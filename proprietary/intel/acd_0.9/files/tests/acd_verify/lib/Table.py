###############################################################################
# INTEL CONFIDENTIAL                                                          #
#                                                                             #
# Copyright 2021 Intel Corporation.                                           #
#                                                                             #
# This software and the related documents are Intel copyrighted materials,    #
# and your use of them is governed by the express license under which they    #
# were provided to you ("License"). Unless the License provides otherwise,    #
# you may not use, modify, copy, publish, distribute, disclose or transmit    #
# this software or the related documents without Intel's prior written        #
# permission.                                                                 #
#                                                                             #
# This software and the related documents are provided as is, with no express #
# or implied warranties, other than those that are expressly stated in the    #
# License.                                                                    #
###############################################################################

from lib.Region import Region

import warnings


class Table(Region):
    def processRequest(self, request, report):
        if "table" in request["regions"]:
            tableInfo = {}
            errorList = {}

            for key in request["sections"]:
                if key == "metadata":
                    metadataObj = request["sections"]["metadata"]
                    if hasattr(metadataObj, 'getTableInfo'):
                        tableInfo["metadata"] = metadataObj.getTableInfo()
                        errorList["metadata"] = metadataObj.getErrorList()

                elif ("cpu" in key) and ('dies' not in report):
                    tableInfo[key] = {}
                    errorList[key] = {}
                    self.processSectionOf(
                        request["sections"][key],
                        tableInfo[key],
                        errorList[key])
                
                elif ("cpu" in key) and ('dies' in report):
                    tableInfo[key] = {}
                    errorList[key] = {}
                    for die in request["sections"][key]:
                        tableInfo[key][die] = {}
                        errorList[key][die] = {}
                        self.processSectionOf(
                            request["sections"][key][die],
                            tableInfo[key][die],
                            errorList[key][die])

            report["table"] = tableInfo
            report["tableInfo"] = errorList

            return True

    def processSectionOf(self, requestSections, tableInfoSections, errorListSections):
        for section in requestSections:
            sectionObj = requestSections[section]
            if section == "mca":
                if hasattr(sectionObj, 'getTableInfo'):
                    tables = sectionObj.getTableInfo()
                    errors = sectionObj.getErrorList()

                    if tables[0]:
                        tableInfoSections["mca_core"] = tables[0]
                    if tables[1]:
                        tableInfoSections["mca_uncore"] = tables[1]
                    if tables[2]:
                        tableInfoSections["mca_soc"] = tables[2]

                    errorListSections["mca_core"] = errors[0]
                    errorListSections["mca_uncore"] = errors[1]
                    errorListSections["mca_soc"] = errors[2]
            elif section == "uncore":
                if hasattr(sectionObj, 'getTableInfo'):
                    tables = sectionObj.getTableInfo()
                    if type(tables) == list:
                        for bdf in tables:
                            bdfName = bdf['Section']
                            if 'regsWE' in bdf:
                                bdf.pop('regsWE')
                            tableInfoSections[bdfName] = bdf
                    errorListSections[section] = sectionObj.getErrorList()
            else:
                if hasattr(sectionObj, 'getTableInfo'):
                    tableInfoSections[section] = sectionObj.getTableInfo()
                    errorListSections[section] = sectionObj.getErrorList()
