...
class Uncore(Section):
    ...
    def search(self, key, value):
        ...
        if type(value) == dict:
            ...
            if not key.startswith('_'):
                ...
                offset = lastKey.split('_')[-1].lower()
                ...
                elif bdf.startswith("RDIAMSR"):
                    self.rdiamsrs["total"] += 1

                if self.eHandler.isError(value):
                    error = self.eHandler.extractError(value)
                    self.eHandler.errors[key] = error

                    if bdf.startswith("RDIAMSR"):
                        self.rdiamsrs["regsWE"] += 1
                        self.rdiamsrs["offsetsWE"].append(error)
                    else:
                ...
                io0CapID2 = (bdf == 'B30_D05_F0') and (offset == '0x28c')
                io1CapID2 = (bdf == 'B30_D09_F0') and (offset == '0x28c')
    ...
    def getTableInfo(self):
        ...
        if self.rdiamsrs["total"] > 0:
            tableInfo = {
                "Section": f"{self.sectionName}-RDIAMSR",
                "rootNodes": f"{self.rdiamsrs['total']}",
                "regsWE":  ['regsWE']
            }

            hasErrors = len(self.rdiamsrs['offsetsWE']) > 0
            if hasErrors:
                level = dict(collections.Counter(self.rdiamsrs['offsetsWE']))
                for error in level:
                    if error not in tableInfo:
                        percentage = round((level[error]*100)/self.rdiamsrs['total'], 1)
                        tableInfo[error] = f"{percentage}% ({level[error]})"
            lUncoreObjs.insert(0, tableInfo)
    ...
    def getChop(self):
        ...
        # Deprecate the previous SPR and ICX check
        pass
