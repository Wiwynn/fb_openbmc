...
import warnings
import importlib
from lib.NodeNameLoader import apply_node_name_from_excel
...
class Postprocessor():
    ...
    def txtPECIFailTable(self, table, dies=None):
        tInfo = {}
        ...
        apply_node_name_from_excel(headers, tInfo, self.report['uncoreRegsDict'])
        
        columns = [tInfo[x] for x in tInfo]
        return f"PECI CC Fail Table \n {self.txtTable(headers, columns)}"
    ...
    def txtSelfCheck(self, errorList, dies=None):
        ...
        **Note: Time Information which conforms Time Table is also
        ...
        if 'upiCountCheck' in self.report:
            for cpu in self.report['upiCountCheck']:
                for die in self.report['upiCountCheck'][cpu]:
                    if die.startswith('compute') or die.startswith('soc'):
                        continue
                    sections = self.report['upiCountCheck'][cpu][die]
                    hasMCA = 'mca' in sections
                    hasUncore = 'uncore' in sections
                    hasSections = hasMCA and hasUncore

                    if not hasSections:
                        error = ''
                        if 'mca' not in sections:
                            error = "MCA-Uncore not in file, UPI banks "+\
                                    "not detected in file"
                            sErrors += f"\n\t\t{cpu}.{die}: {error}"
                            errors += 1
                        continue
                    if hasSections:
                        mcaIsNone = sections['mca'] == None
                        uncoreIsNone = sections['uncore'] == None

                        if mcaIsNone or uncoreIsNone:
                            error = ''
                            if mcaIsNone:
                                error = "\n\t\tMCA Uncore has no UPI Banks in " +\
                                        f"{cpu}.{die}"
                            if uncoreIsNone:
                                error = f"\n\t\t{cpu}.{die} Uncore does not " +\
                                        "have CAPID2 reg"
                            sErrors += error
                            errors += 1

                        elif sections['mca'] != sections['uncore']:
                            error = "\n\t\tAccording to CAPID2, "+\
                                    f"{sections['uncore']} " +\
                                    "UPI Links should be enabled but only " +\
                                    f"{sections['mca']} show enabled in " +\
                                    f"{cpu}.{die} MCA Uncore"
                            sErrors += error
                            errors += 1
                        else:
                            msg = "\n\t\tAmount of UPI Links enabled agrees with " +\
                                  f"CAPID2 in {cpu}.{die}"
                            sErrors += msg
    ...
    def txtTimesTable(self, timeInfo, dies=None):
        ...
        columns = [[], [], []]
        total_time_key_count = 0

        _total_time = timeInfo["metadata"].pop('_total_time', None)
        columns[0].append("_total_time")
        columns[1].append("_total_time")
        if _total_time is None:
            columns[2].append("Missing Key")
        else:
            columns[2].append(str(_total_time))
        ...
        return f"\n\n\nTime Table ('_time*' keys count: {total_time_key_count})\n {self.txtTable(headers, columns)}"
