...
class Big_core(Section):
    ...
    def checkSize(self, key, value):
        ...
        _gnrSize = (value.upper().startswith("0X05B8"))
        _gnrSizeWithSq = (value.upper().startswith("0X040805B0"))
        gnrValidSize = _gnrSize or _gnrSizeWithSq

        if (not icxValidSize) and (not sprValidSize) and (not gnrValidSize):
