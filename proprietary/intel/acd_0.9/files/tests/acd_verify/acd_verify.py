...
import lib.SchemaCheck as SchemaCheck
...
def processJsonFile(rProcess, fVerbose=False, fCompare=False, rCompare='',
    ...
    try:
        ...
        schemaCheckReport = ""
        if "skipSchemaCheck" not in checks:
            schemaCheckReport = SchemaCheck.collect_all_messages_chk(rProcess, acdData)
            if "skipverifycheck" in checks:
                sReport = "schemaCheck" + "\n\n" + schemaCheckReport
                try:
                    #save to == rProcess replace .json with .txt
                    saveTo = os.path.splitext(rProcess)[0] + ".txt"
                    f = open(saveTo, "w")
                    f.write(sReport)
                    f.close()
                    print(f"Report saved:{saveTo}")
                except Exception as e:
                    print(f"Write to:{rProcess} failed: {e}")              
                return 
        ...
        if "skipSchemaCheck" not in checks:
            processor.report["schemaCheck"] = schemaCheckReport

        version = acdData.get("METADATA", "").get("_version", "")
        sku = getSKU(version)
        if sku == "GNR" or sku == "GNRD":
            processor.report["uncoreRegsDict"] = "NodeNameGNR"
        elif sku == "SRF":
            processor.report["uncoreRegsDict"] = "NodeNameSRF"
        else:
            processor.report["uncoreRegsDict"] = None
            print("SKU not recognized, node name dict not set.")
...
def main():
    try:
        ...
        parser.add_argument("--skip_schema_check",
                            action = 'store_true',
                            help = "Skip json file schema check")
        parser.add_argument("--skip_verify_check",
                            action = 'store_true',
                            help = "Skip json file ACD verify check")      
        ...
        if args.skip_schema_check:
            checks["skipSchemaCheck"] = None
        if args.skip_verify_check:
            checks["skipverifycheck"] = None
