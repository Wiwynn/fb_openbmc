...
class InputParserTestFixture : public ::testing::Test
{
    void SetUp() override
    {
        ...
        cmdInOut.rmSectionInfo->isRemoteSection = false;
        cmdInOut.rmSectionInfo->instanceLoops = 0;
        cmdInOut.rmSectionInfo->modulePath = NULL;
        cmdInOut.rmSectionInfo->rmModuleInfo = &rmModuleInfo;
    }
  ...
  public:
    ...
    CmdInOut cmdInOut = {};
    RemoteModuleSectionInfo rmSectionInfo = {};
...
TEST_F(InputParserTestFixture, RM_Section_Parsing)
{
    cmdInOut.rmSectionInfo->isRemoteSection = true;

    LoggerStruct loggerStruct;
    cmdInOut.in.params =
        cJSON_Parse(R"(["Target", "RandomString", 0, false, true, 10])");
    status = UpdateParams(&cpus[0], &cmdInOut, &loggerStruct, &errInfo);
    EXPECT_EQ(ACD_SUCCESS, status);

    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->intCount == 3);
    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->intArg[0] == 48);
    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->intArg[1] == 0);
    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->intArg[2] == 10);

    int result;
    strcmp_s(cmdInOut.rmSectionInfo->rmModuleInfo->strArg[0], 64,
             "RandomString", &result);
    EXPECT_TRUE(result == 0);

    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->boolArg[0] == false);
    EXPECT_TRUE(cmdInOut.rmSectionInfo->rmModuleInfo->boolArg[1] == true);
}
