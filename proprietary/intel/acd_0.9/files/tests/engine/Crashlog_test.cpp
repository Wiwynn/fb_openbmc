...
//                         SetArgPointee<8>(cc), Return(PECI_CC_SUCCESS)));
//
//     uint8_t returnValue[8] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
//     0x07}; uint8_t returnValue1[8] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
//     0x16, 0x17}; uint8_t returnValue2[8] = {0x20, 0x21, 0x22, 0x23, 0x24,
//     0x25, 0x26, 0x27}; cc = 0x40; EXPECT_CALL(*crashdump.libPeciMock,
...
//         .WillOnce(DoAll(SetArrayArgument<5>(returnValue2, returnValue2 + 8),
//                         SetArgPointee<6>(cc), Return(PECI_CC_SUCCESS)))
//         .WillRepeatedly(DoAll(SetArrayArgument<5>(returnValue, returnValue +
//         8),
//                               SetArgPointee<6>(cc),
...
TEST(CrashlogTest, Crashlog_isCpuCrashlogEnabled_2Cpu_enabled_disabled)
{
    TestCrashdump crashdump(cd_gnr, 0, 2);

    uint8_t cc = 0x40;
    uint8_t DataSample0[8] = {0x00, 0x00, 0x00, 0x70, 0x00, 0x00, 0x00, 0x00};
    uint8_t DataSample1[8] = {0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00};

    EXPECT_CALL(*crashdump.libPeciMock, peci_Telemetry_ConfigWatcherRd_dom)
        .WillOnce(DoAll(SetArrayArgument<5>(DataSample0, DataSample0 + 8),
                        SetArgPointee<6>(cc), Return(PECI_CC_SUCCESS)))
        .WillOnce(DoAll(SetArrayArgument<5>(DataSample1, DataSample1 + 8),
                        SetArgPointee<6>(cc), Return(PECI_CC_SUCCESS)));

    bool enabled = isCpuCrashlogEnabled_dom(0, crashdump.cpus[0].clientAddr);
    EXPECT_TRUE(enabled);
    enabled = isCpuCrashlogEnabled_dom(0, crashdump.cpus[1].clientAddr);
