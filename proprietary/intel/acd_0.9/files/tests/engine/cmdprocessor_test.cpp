...
class ProcessCmdTestFixture : public ::testing::Test
{
  protected:
    ...
    "CrashDump_Discovery_dom": [{"Params": [48, 9, 2, 3, 4, 5, 8]}],
...
TEST_F(ProcessCmdTestFixture, APIs_Valid_test)
{
    ...
    cJSON* peciParams;
    CPUInfo* cpuInfo = cpus;
    cpus[0].dieMaskInfo.dieMask = 0x211;
...
TEST_F(ProcessCmdTestFixture, InternalVar)
{
    CmdInOut cmdInOut;
    CPUInfo cpuInfo;
    cJSON* internalVar;
    cmdInOut.out.ret = PECI_CC_TIMEOUT;
    cmdInOut.out.cc = 0x40;
    cmdInOut.internalVarsTracker = cJSON_CreateObject();
    cmdInOut.internalVarName = NULL;
    cmdInOut.out.size = sizeof(uint32_t);

    // PECI Failure ; stored as CMD_ERROR_VALUE
    cmdInOut.internalVarName = "Var1";
    cmdInOut.out.ret = PECI_CC_TIMEOUT;
    UpdateInternalVar(&cmdInOut, &cpuInfo);
    internalVar = cJSON_GetObjectItemCaseSensitive(cmdInOut.internalVarsTracker,
                                                   cmdInOut.internalVarName);
    ASSERT_TRUE(internalVar != NULL);

    // PECI Success; 32 bit value
    cmdInOut.internalVarName = "Var2";
    cmdInOut.out.val.u32[0] = 0x80000000;
    ...
    // PECI Success; 64 bit value
    cmdInOut.internalVarName = "Var3";
    cmdInOut.out.size = sizeof(uint64_t);
    cmdInOut.out.val.u64 = 0x8000000000000000;
    cmdInOut.out.ret = PECI_CC_SUCCESS;
    UpdateInternalVar(&cmdInOut, &cpuInfo);
    internalVar = cJSON_GetObjectItemCaseSensitive(cmdInOut.internalVarsTracker,
                                                   cmdInOut.internalVarName);
