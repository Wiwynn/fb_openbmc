...
class LibPeciMock
{
  public:
    ...
    MOCK_METHOD9(peci_CrashDump_Discovery_dom,
    ...
    MOCK_METHOD8(peci_CrashDump_GetFrame_dom,
                 EPECIStatus(uint8_t, uint8_t, uint16_t, uint16_t, uint16_t,
                             uint8_t, uint8_t*, uint8_t*));
    ...
    MOCK_METHOD(EPECIStatus, peci_RdEndPointConfigPciLocal_seq_dom,
                (uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint16_t,
                 uint8_t, uint8_t*, int, uint8_t*));

    MOCK_METHOD(EPECIStatus, peci_RdEndPointConfigMmio_dom,
    ...
    MOCK_METHOD(EPECIStatus, peci_RdEndPointConfigPciLocal_dom,
                (uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint16_t,
                 uint8_t, uint8_t*, uint8_t*));
    ...
    MOCK_METHOD9(peci_WrEndPointPCIConfigLocal,
                 EPECIStatus(uint8_t, uint8_t, uint8_t, uint8_t, uint8_t,
                             uint16_t, uint8_t, uint32_t, uint8_t*));

    MOCK_METHOD(EPECIStatus, peci_WrEndPointPCIConfigLocal_dom,
                (uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint16_t,
                 uint8_t, uint32_t, uint8_t*));

    MOCK_METHOD(EPECIStatus, peci_WrPkgConfig_dom,
                (uint8_t, uint8_t, uint8_t, uint16_t, const void*, uint8_t,
                 uint8_t*));

    MOCK_METHOD(EPECIStatus, peci_WrPkgConfig,
                (uint8_t, uint8_t, uint16_t, const void*, uint8_t, uint8_t*));

    MOCK_METHOD9(peci_Telemetry_Discovery_dom,
                 EPECIStatus(uint8_t, uint8_t, uint8_t, uint8_t, uint16_t,
                             uint8_t, uint8_t, uint8_t*, uint8_t*));
    ...
    MOCK_METHOD7(peci_Telemetry_GetCrashlogSample_dom,
                 EPECIStatus(uint8_t, uint8_t, uint16_t, uint16_t, uint8_t,
                             uint8_t*, uint8_t*));
    ...
    MOCK_METHOD7(peci_Telemetry_ConfigWatcherRd_dom,
                 EPECIStatus(uint8_t, uint8_t, uint16_t, uint16_t, uint8_t,
                             uint8_t*, uint8_t*));
    ...
    MOCK_METHOD7(peci_Telemetry_ConfigWatcherWr_dom,
                 EPECIStatus(uint8_t, uint8_t, uint16_t, uint16_t, uint8_t,
                             uint8_t*, uint8_t*));
    ...
    MOCK_METHOD6(peci_Telemetry_ConfigWatcherWr,
                 EPECIStatus(uint8_t, uint16_t, uint16_t, uint8_t, uint8_t*,
                             uint8_t*));
    MOCK_METHOD(EPECIStatus, peci_WrEndPointConfigMmio_dom,
                (uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t,
                 uint8_t, uint64_t, uint8_t, uint64_t, uint8_t*));
    ...
    void DelegateWrEndPointConfigMmio_dom()
    {
        // Use WillByDefault with a lambda function to dynamically change
        // arguments
        ON_CALL(*this, peci_WrEndPointConfigMmio_dom)
            .WillByDefault([=](uint8_t target, uint8_t domainId, uint8_t u8Seg,
                               uint8_t u8Bus, uint8_t u8Device, uint8_t u8Fcn,
                               uint8_t u8Bar, uint8_t u8AddrType,
                               uint64_t u64Offset, uint8_t u8DataLen,
                               uint64_t u64DataVal,
                               uint8_t* cc) -> EPECIStatus {
                // TPMI BMC_CTL_INTERFACE
                if ((u8Bus == 0) && (u8Device == 3) && (u8Fcn == 7))
                {
                ...
                *cc = PECI_DEV_CC_SUCCESS;
                ...
                // Example: Return a specific status based on the input
                // arguments
                return PECI_CC_SUCCESS;
            });
    }
    void DelegateRdEndPointConfigMmio_dom()
    {
        // Use WillByDefault with a lambda function to dynamically change
        // arguments
        ON_CALL(*this, peci_RdEndPointConfigMmio_dom)
            .WillByDefault([=](uint8_t target, uint8_t domainId, uint8_t u8Seg,
                               uint8_t u8Bus, uint8_t u8Device, uint8_t u8Fcn,
                               uint8_t u8Bar, uint8_t u8AddrType,
                               uint64_t u64Offset, uint8_t u8ReadLen,
                               uint8_t* pMmioData, uint8_t* cc) -> EPECIStatus {
                // TPMI BMC_CTL_DATA
                if ((u8Bus == 0) && (u8Device == 3) && (u8Fcn == 7))
                {
                    // Physical to Logical will follow each other
                    *pMmioData = 2;
                    *(pMmioData + 1) = 3;
                }
    ...
    void DelegateRdEndPointConfigPciLocal_dom()
    {
        // Use WillByDefault with a lambda function to dynamically change
        // arguments
        ON_CALL(*this, peci_RdEndPointConfigPciLocal_dom)
            .WillByDefault([=](uint8_t target, uint8_t domainId, uint8_t u8Seg,
                               uint8_t u8Bus, uint8_t u8Device, uint8_t u8Fcn,
                               uint16_t u16Reg, uint8_t u8ReadLen,
                               uint8_t* pPCIData, uint8_t* cc) -> EPECIStatus {
                // BusEnumeration
                if ((u8Bus == 8) && (u8Device == 3) && (u8Fcn == 0))
                {
                    *pPCIData = 8; // Random value for now
                    *cc = (uint8_t)PECI_DEV_CC_SUCCESS;
                }
...
{
  public:
    explicit LibPeciMockWithDelay(int delay)
    {
    ...
    void DelegateForBigCore()
    {
        ...
        ON_CALL(*this, peci_CrashDump_GetFrame_dom)
            .WillByDefault([this](uint8_t target, uint8_t domainID,
                                  uint16_t param0, uint16_t param1,
                                  uint16_t param2, uint8_t u8ReadLen,
                                  uint8_t* pData, uint8_t* cc) {
                return peciWithDelay_->peci_CrashDump_GetFrame_dom(
                    target, domainID, param0, param1, param2, u8ReadLen, pData,
                    cc);
            });
    }
        ...
        ON_CALL(*this, peci_CrashDump_GetFrame_dom)
            .WillByDefault([this](uint8_t target, uint8_t domainID,
                ...
                return peciWithDelay_
                    ->peci_CrashDump_GetFrame_dom_GoodReturnWithDelay(
                        target, domainID, param0, param1, param2, u8ReadLen,
                        pData, cc);
            });
    }
    ...
    void DelegateForBigCoreWithGoodReturnNeedRetryCompletionCodeWithDelay()
    {
        ...
        ON_CALL(*this, peci_CrashDump_GetFrame_dom)
            .WillByDefault([this](uint8_t target, uint8_t domainID,
                                  ...
                                  uint8_t* pData, uint8_t* cc) {
                return peciWithDelay_
                    ->peci_CrashDump_GetFrame_dom_GoodReturnRetryCCWithDelay(
                        target, domainID, param0, param1, param2, u8ReadLen,
                        pData, cc);
            });
    }
