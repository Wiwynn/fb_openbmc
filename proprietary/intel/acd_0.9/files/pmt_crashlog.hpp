...
class PMTCrashlog
{
  private:
    ...
    std::unique_ptr<pmt::transport::TransportIface> mctpTransport;
    std::chrono::time_point<std::chrono::system_clock> lastUpdTime;
    std::set<std::shared_ptr<pmt::transport::EndpointIface>> endpoints{};
    inline void setNACFlag(bool status);
    void pmtCollectCrash(cJSON* const pJsonChild, boost::asio::io_service& io);
    bool
        getCrashDetails(boost::asio::yield_context yield,
