...
install_sysv() {
  ...
  install -m 755 ${UNPACKDIR}/setup_crond.sh ${D}${sysconfdir}/init.d/setup_crond.sh
...
install_systemd() {
  ...
  install -m 755 ${UNPACKDIR}/setup_crond.sh ${D}${prefix}/local/bin/setup_crond.sh
  install -m 644 ${UNPACKDIR}/setup_crond.service ${D}${systemd_system_unitdir}/setup_crond.service
...
do_install:append:openbmc-fb() {
  ...
  install -m 755 ${UNPACKDIR}/cron.daily/biosfwimages-cleanup.sh ${D}${sysconfdir}/cron.daily/biosfwimages-cleanup.sh
