...
class GTVrComponent : public VrComponent, public SignComponent {
  public:
    ...
    int component_update(const string& image, bool force) { return VrComponent::_update(image, force); }
};

int GTVrComponent::update(const string& image) {
  return signed_image_update(image, false);
}
int GTVrComponent::fupdate(const string& image) {
